...
#         self.conv = nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0, has_bias=False)
...
class MultiHeadSelfAttention(nn.Cell):
    def __init__(self, embed_size, heads, dropout_rate=0.1):
        ...
        self.values = nn.Dense(self.head_dim, self.head_dim, has_bias=False)
        self.keys = nn.Dense(self.head_dim, self.head_dim, has_bias=False)
        self.queries = nn.Dense(self.head_dim, self.head_dim, has_bias=False)
        self.fc_out = nn.Dense(heads * self.head_dim, embed_size)
        self.dropout = nn.Dropout(1 - dropout_rate)
        self.scale = P.Pow()(Tensor(self.head_dim), Tensor(-0.5))
    ...
    def construct(self, x):
        ...
        # 分割输入到每个头
        x = x.view(N, H * W, self.heads, self.head_dim)

        values = self.values(x.view(-1, self.head_dim)).view(N, H * W, self.heads, self.head_dim)
        keys = self.keys(x.view(-1, self.head_dim)).view(N, H * W, self.heads, self.head_dim)
        queries = self.queries(x.view(-1, self.head_dim)).view(N, H * W, self.heads, self.head_dim)

        keys = keys.transpose(2, 3)
        queries = queries.transpose(2, 3)

        # 计算注意力
        attention = P.BatchMatMul(queries, keys) * self.scale
        attention = P.Softmax(axis=-1)(attention)
        attention = self.dropout(attention)

        # 重新组合多个头的输出
        attention = P.BatchMatMul(attention, values)
        attention = attention.transpose(2, 3).reshape(N, H * W, -1)
...
class NoResidualBlockBase(nn.Cell):
    """
    Modified ResNet V1 block without residual connections.
    ...
    def __init__(self,
                 ...
                 se_block=False,
                 res_base=True):
        super(NoResidualBlockBase, self).__init__()
    ...
    def construct(self, x):
        ...
        # The residual connection (skip connection) is removed
        out = self.relu(out)
...
class ResNetAttention(nn.Cell):
    ...
    def __init__(self,
        ...
        # self.attention = Attention(out_channels[3], out_channels[3])  # 添加attention层
    ...
    def construct(self, x):
        ...
        # 应用自注意力层
        # 注意：根据自注意力层的具体实现，可能需要对 c5 进行重塑或其他预处理
        # attention_output = self.attention_layer(c5)
        attention_output = self.attention(c5)

        # 使用注意力层的输出作为全连接层的输入
        out = self.mean(attention_output, (2, 3))
        out = self.flatten(out)
        out = self.end_point(out)
...
def resnet50_attention(class_num=10,embed_size=2048,heads=16):
    """
    Get ResNet50 neural network.
    ...
    Examples:
        >>> net = resnet50(10)
    """
    return ResNetAttention(ResidualBlock,
                  [3, 4, 6, 3],
                  [64, 256, 512, 1024],
                  [256, 512, 1024, 2048],
                  [1, 2, 2, 2],
...
def cnn50(class_num=10):
    ...
    Examples:
        >>> net = resnet50(10)
    """
    return ResNet(NoResidualBlockBase,
                  [3, 4, 6, 3],
                  [64, 256, 512, 1024],
                  [256, 512, 1024, 2048],
                  [1, 2, 2, 2],
